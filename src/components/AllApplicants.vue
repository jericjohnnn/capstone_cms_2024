  ...
  <div class="flex flex-col">
    <div class="mb-6">
      ...
      >
        <!-- Header Row -->
        <div
          class="flex font-semibold text-gray-700 bg-gray-50 justify-between py-3 px-4 border-b"
        >
          ...
          <div class="w-28 text-center">Credentials</div>
        ...
        <div class="overflow-y-auto max-h-[calc(100vh-16rem)]">
          ...
          >
            <div class="w-10 text-center font-medium text-gray-700">
              {{ tutor.user_id }}
            </div>
            <div class="w-28 text-center font-medium">
              {{ tutor.first_name }} {{ tutor.last_name }}
            </div>
            <div class="w-16 text-center">
              <img
                :src="tutor.profile_image || defaultAvatar"
                alt="Profile"
            ...
            <div class="w-20 text-center font-medium">
              ₱{{ tutor.tutor_rate }}
            </div>
            <div class="w-28 text-center">
              <div class="card flex justify-center">
                <Drawer
                  ...
                  header="Credentials"
                  position="full"
                  pt:content="!flex !w-full !flex-wrap !gap-10"
                >
                  <div v-if="images.length === 0">
                    NO CREDENTIALS SUBMITED
                  </div>
                  <div
                    v-else
                ...
                <Button icon="pi pi-window-maximize" @click="replaceImage(tutor.id)" />
              </div>
            </div>
            <div class="w-28 text-center">
              <button
...
const replaceImage = (tutorId) => {
  visible.value = true
  images.value = tutors.value.find(tutor => tutor.id === tutorId).credentials
}
...
async function fetchTutors(query = '') {
  try {
    const response = await axiosInstance.get(
      `/api/admin/all-tutors?search=${query}`,
    )
...
const filteredTutors = computed(() => {
  ...
  if (currentTab.value !== 'all') {
    filtered = filtered.filter(
      tutor => tutor.approval_status === capitalize(currentTab.value),
    )
  }
